...
**** Signals calculate /n/ numbers per second.
     ...
     - Audio rate :: A separate calculation for every audio output sample.
     - Control rate :: Fewer calculations (saves CPU for slower-moving signals).
     - Initial rate :: Calculates once, and holds forever.
